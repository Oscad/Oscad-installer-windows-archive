...
class ProjectParam(template.MyTemplate):
  ...
  # Call all pending idle tasks, without processing any other events.
    self.update_idletasks()
   # command="cmd -e \""+self.OSCAD_HOME+"/kicadtoNgspice/KicadtoNgspice.py "+self.projectName+".cir 1\""
    command="python "+self.OSCAD_HOME+"/kicadtoNgspice/KicadtoNgspice.py "+self.projectName+".cir 1"
    try:
        thread.start_new_thread(self.call_system,(command,))
    except Exception,err:
        print err
  ...
  # Call all pending idle tasks, without processing any other events.
    self.update_idletasks()
    command="python "+ self.OSCAD_HOME+"/analysisInserter/convertgui.py"
    try:
        thread.start_new_thread(self.call_system,(command,))
    except Exception,err:
        print err
  ...
  # Call all pending idle tasks, without processing any other events.
    self.update_idletasks()
    command="python "+self.OSCAD_HOME+"/modelEditor/modelEditor.py " +self.projectName+".cir 1"
  ...
  # Call all pending idle tasks, without processing any other events.
    self.update_idletasks()
    command="python "+self.OSCAD_HOME+"/subcktEditor/subcktEditor.py " +self.projectName+".cir "
    try:
        thread.start_new_thread(self.call_system,(command,))
    except Exception,err:
        print err
  ...
  # Call all pending idle tasks, without processing any other events.
    ...
    command="wscilex -f " +self.OSCAD_HOME+"/LPCSim/LPCSim/Main.sci -args "+self.projectName+".cir.ckt "
    try:
        thread.start_new_thread(self.call_system,(command,))
    except Exception,err:
        print err
